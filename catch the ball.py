...
with mp_hands.Hands(min_detection_confidence = 0.3, min_tracking_confidence = 0.3) as hand:
    while cap.isOpened():
        ret, frame = cap.read()
        ...
        if results.multi_hand_landmarks:
            ...
            for id,lm in enumerate(hand_landmark.landmark):
                cx,cy = int(lm.x*400), int(lm.y*400)
                f=[cx,cy]
                #temp=[x,y]
                if id==5:
                    a=[cx,cy]
                if id==8:
                    b=[cx,cy]
                if id in [8,12,16,20,4]:
                    if math.sqrt(((f[0]-x)**2)+((f[1]-y)**2))<=71:
                        x,y=f[0],f[1]
                        #vx,vy=(temp[0]-x),(temp[1]-y)
